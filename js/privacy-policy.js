...
function renderPolicy(page, policy, site) {
      ...
      <div class="policy-grid">
        ${(policy.sections || []).map((section) => `
          <article class="policy__section">
            ...
            ${section.phone ? `<p><a class="contact-phone" href="tel:${escapeHtml(site.phoneTel || section.phone)}">Call ${escapeHtml(section.phone)}</a></p>` : ""}
...
async function init() {
  ...
  applyPageMeta(policies.seo.privacy);
  renderShell(site);
  renderPolicy("Privacy policy", policies.privacy, site);
